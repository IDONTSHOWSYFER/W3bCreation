...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  setTimeout(()=> bar.hidden = false, 6000); // apparaît après 6 s
});

/* ---- Tilt 3D logos CMS ---- */
document.addEventListener('DOMContentLoaded', () => {
  VanillaTilt.init(
    document.querySelectorAll('.cms-logos img'),
    {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const onScroll = () => {
    ...
    if (maxScroll < 200) { window.removeEventListener('scroll', onScroll); return; }
  ...
  btn  .addEventListener('click', () => window.open(
       'https://view.forms.app/seo-checklist', '_blank'));
});
...
document.addEventListener('DOMContentLoaded', ()=>{
  gsap.utils.toArray('.hero-layer').forEach((layer,i)=>{
    gsap.fromTo(layer,{y:-40*i},{y:40*i,
      ease:'none',
      scrollTrigger:{
        trigger:'#hero',
        start:'top top',
...
document.addEventListener('DOMContentLoaded',()=>{
  gsap.from('.arguments-columns .pros', {x:-60, opacity:0, duration:.8,
        scrollTrigger:{trigger:'.arguments-columns',start:'top 80%'}});
  gsap.from('.arguments-columns .cons', {x:60, opacity:0, duration:.8,
        scrollTrigger:{trigger:'.arguments-columns',start:'top 80%'}});
  gsap.from('.cms-logos img', {scale:.5, opacity:0, stagger:.1, duration:.6,
